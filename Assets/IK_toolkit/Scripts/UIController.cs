...
public class UIController : MonoBehaviour
{
    ...
    public float rotMultiplier = 1.0f;
                ...
                using (StreamReader sr = new StreamReader(fs))
                {
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        try
                        {
                            ...
                            // Posigion Parsing -> { 0, -0.41, 0.71, 1.08), 0.5, True}
                            ...
                            // rotation Parsing
                            int indexOpenBraket2 = line.IndexOf('(', indexCloseBraket + 1);
                            int indexCloseBraket2 = line.IndexOf(')', indexOpenBraket2 + 1);
                            string rotation = line.Substring(indexOpenBraket2 + 1, indexCloseBraket2 - indexOpenBraket2 - 1);

                            string leftOver = line.Remove(0, indexCloseBraket2 + 2);
                        ...
                        // bool isInt =int.TryParse(step[0], out data.stepNum);

                        //if (!isInt)
                        //    Debug.LogWarning($"{teachDataPath}의 {0}번째 데이터에 오류가 있습니다.");
...
void Update()
    ...
    {
        isZRotMinusClicking = true;
    }

    public void OnZRotMinusBtnUpEvent()
    {
        isZRotMinusClicking = false;
    }
}
